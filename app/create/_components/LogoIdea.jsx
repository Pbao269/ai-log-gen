...
function LogoIdea({formData,onHanldleInputChange}) {
  ...
  return (
    ...
    <div className='flex flex-wrap gap-3 mt-6'>
      {ideas&&ideas.map((item,index)=>(
        <h2 key={index}
        onClick={()=>{setSelectedOption(item.idea);
          onHanldleInputChange(item.idea)
        }}
      ...
      <h2 
       onClick={()=>{setSelectedOption('Let AI Select the best idea');
        onHanldleInputChange('Let AI Select the best idea')
      }}
      className={`p-2 rounded-full border px-3 cursor-pointer
